...
module.exports = function (inOptions, inBackend) {
  ...
  let agentsAllOptions = inOptions.agents
  agentsAllOptions.agents.forEach((agent) => {
    if (agent.id === undefined) { agent.id = Tools.generateId() }
  })
  ...
  app.get('/api/get-agent', (req, res) => {
    ...
    if (agent === undefined) { return res.sendStatus(400, 'Not found') }
    res.json(agent)
  })
  ...
  app.get('/api/remove-agent', (req, res) => {
    var id = req.query.id
    var agentIdx = agentsAllOptions.agents.findIndex((agent) => { return agent.id === id })
    if (agentIdx === -1) { return res.sendStatus(400, 'Not found') }
    agentsAllOptions.agents.splice(agentIdx, 1, req.body)
    res.sendStatus(200)
  })
}
