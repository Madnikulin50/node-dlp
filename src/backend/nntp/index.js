...
class NNTP {
  ...
  initServer () {
    ...
    nntp._getGroups = async (session, ts, wildmat) => {
      let selg = groupsDef.filter(group => wildmat === undefined ? true : group.description.indexOf(wildmat) !== -1)
    ...
    nntp._selectGroup = async (session, groupId) => {
      let grp = groupsDef.filter(g => g.name === groupId)[0]
      if (grp === undefined) { return Promise.reject(new Error('Group not found')) }
      const getNumIncidentsAsync = promisify(this.store.getNumIncidents.bind(this.store))
      ...
      if (session.group !== undefined) { session.group = Object.assign({}, grp) }
     ...
     e.from === null) {
          if (e.user !== undefined &&
      e.user !== null) { e.from = e.user } else if (e.src_ip !== undefined &&
      e.src_ip !== null) { e.from = e.src_ip }
        }
        return e
      })
    ...
    nntp._getArticle = async (session, messageId) => {
      let match, msg

      match = messageId.match(/^<([^<>]+)>$/)
      ...
      if (msg === undefined ||
    msg.length === 0) { return Promise.reject(new Error('Message not found')) }
      msg = msg[0]
    ...
    nntp._buildHead = (session, msg) => {
      ...
      for (let i in msg) {
        if (ignored.findIndex(item => i === item) !== -1) { continue }
    ...
    nntp._buildHeaderField = function (session, msg, field) {
      if (msg[field] !== undefined) { return msg[field] }
      switch (field) {
        case 'from':
        {
          if (msg.from) { return msg.from }
          if (msg.user !== undefined) { return msg.user }
          if (msg.src_ip !== undefined) { return msg.src_ip }
